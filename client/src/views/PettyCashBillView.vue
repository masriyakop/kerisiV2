...
import { listPettyCashBills } from "@/api/cms";
import { useToast } from "@/composables/useToast";
import type { PettyCashBillRow } from "@/types";

const PAGE_NAME = "Bill Petty Cash";
const PAGE_BREADCRUMB = "Petty Cash / Bill Petty Cash";

const toast = useToast();

const rows = ref<PettyCashBillRow[]>([]);
...
const columns: FimsColumn<PettyCashBillRow>[] = [
  ...
  { key: "pcbBatchAmt", label: "Batch Amount", sortable: true, sortKey: "pcb_batch_amt", hideable: true, align: "right", value: (r) => formatMoney(r.pcbBatchAmt) },
  { key: "pcbStatus", label: "Status", sortable: true, sortKey: "pcb_status", hideable: true, value: (r) => r.pcbStatus ?? "" },
...
async function loadRows() {
  ...
  try {
    ...
    const res = await listPettyCashBills(`?${params.toString()}`);
    rows.value = res.data;
...
const {
  ...
  pageName: PAGE_NAME,
  apiDataPath: "/petty-cash/bills",
  defaultExportColumns: ["Batch No", "Batch Amount", "Status"],
    ...
    <div class="space-y-4">
      ...
      <article class="rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <div class="space-y-4 p-4">
          ...
          >
            <template #action="{ row }">
              <a :href="(row as PettyCashBillRow).urlView" class="rounded p-1 text-slate-600 hover:bg-slate-100" title="View">
