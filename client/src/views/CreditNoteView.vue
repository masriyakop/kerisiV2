...
/**
 * Account Receivable / Credit Note (PAGEID 1459, MENUID 1041)
 *
 * Source: FIMS BL `DT_AR_CREDIT_NOTE_LIST` — datatable + concatenated search
 * + cascading delete (credit_note_details -> credit_note_master). Legacy file
 * has the smart-filter block commented out, so this view uses search only.
 *
 * The footer row ("Total Credit Note / Invoice Total / Invoice Balance") is
 * rendered from `meta.footer` returned by the controller.
 */
...
import { deleteCreditNote, listCreditNotes } from "@/api/cms";
import { useToast } from "@/composables/useToast";
import { useConfirmDialog } from "@/composables/useConfirmDialog";
import type { CreditNoteRow, NoteListFooter } from "@/types";
...
type SortKey =
  ...
  | "cim_invoice_no"
  | "cnm_cn_total_amount";
const sortBy = ref<SortKey>("cnm_crnote_no");
...
async function loadRows() {
  ...
  try {
    const res = await listCreditNotes(`?${params.toString()}`);
    rows.value = res.data;
    ...
    footer.value = {
      ...
      creditNoteTotalAmount: Number(f.creditNoteTotalAmount ?? 0),
    };
  } catch (e) {
    toast.error("Load failed", e instanceof Error ? e.message : "Unable to load credit notes.");
  } finally {
    loading.value = false;
  }
}
...
async function handleDelete(row: CreditNoteRow) {
  const ok = await confirm({
    title: "Delete credit note?",
    message: `Delete credit note \u201C${row.creditNoteNo ?? row.id}\u201D and all its line items? This cannot be undone.`,
    confirmText: "Delete",
    destructive: true,
  });
  if (!ok) return;
  ...
  try {
    ...
    toast.success("Deleted", `Credit note ${row.creditNoteNo ?? row.id} removed.`);
    if (rows.value.length === 1 && page.value > 1) page.value -= 1;
    await loadRows();
  } catch (e) {
    toast.error("Delete failed", e instanceof Error ? e.message : "Unable to delete credit note.");
  }
}

function goCreate() {
  void router.push("/admin/kerisi/m/1782");
}

function goEdit(row: CreditNoteRow) {
  void router.push({ path: "/admin/kerisi/m/1782", query: { id: row.id, mode: "edit" } });
}

function goView(row: CreditNoteRow) {
  void router.push({ path: "/admin/kerisi/m/1782", query: { id: row.id, mode: "view" } });
}
...
const {
  ...
  apiDataPath: "/account-receivable/credit-note",
  ...
  getFilteredList: () =>
    rows.value.map((r) => ({
      ...
      "Credit Note Amount": currencyMyr(r.creditNoteTotalAmount),
      Status: r.status ?? "",
    })),
  datatableRef,
  searchKeyword: q,
...
async function exportExcel() {
  try {
    ...
    const ws = wb.addWorksheet("Credit Note");
    ...
    a.href = url;
    a.download = `Credit_Note_${new Date().toISOString().slice(0, 10)}.xlsx`;
    a.click();
...
<template>
  <AdminLayout>
    <div class="mx-auto max-w-7xl space-y-4">
      ...
      />
      <p class="text-base font-semibold text-slate-500">Account Receivable / Credit Note</p>

      <article class="rounded-lg border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-100 px-4 py-3">
          <h1 class="text-base font-semibold text-slate-900">List of Credit Notes</h1>
              ...
              <table class="w-full min-w-[1100px] text-sm">
                <thead class="sticky top-0 bg-slate-50">
                  <tr class="border-b border-slate-200 text-left">
                    ...
                    <th
                      ...
                      @click="toggleSort('cnm_crnote_no')"
                    >
                      Credit Note No
                      <span v-if="sortBy === 'cnm_crnote_no'">{{ sortDir === "asc" ? "↑" : "↓" }}</span>
                    </th>
                    <th
                      class="cursor-pointer px-3 py-2 text-xs font-semibold uppercase"
                      @click="toggleSort('cnm_crnote_date')"
                    >
                      Date
                      <span v-if="sortBy === 'cnm_crnote_date'">{{ sortDir === "asc" ? "↑" : "↓" }}</span>
                    </th>
                    <th
                      class="cursor-pointer px-3 py-2 text-xs font-semibold uppercase"
                      @click="toggleSort('cnm_cust_name')"
                    >
                      Customer
                      <span v-if="sortBy === 'cnm_cust_name'">{{ sortDir === "asc" ? "↑" : "↓" }}</span>
                    </th>
                    <th
                      class="cursor-pointer px-3 py-2 text-xs font-semibold uppercase"
                    ...
                    <th
                      ...
                      @click="toggleSort('cnm_cn_total_amount')"
                    >
                      Credit Note Amount
                      <span v-if="sortBy === 'cnm_cn_total_amount'">{{ sortDir === "asc" ? "↑" : "↓" }}</span>
                    </th>
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-3 py-2 font-medium text-slate-900">{{ row.creditNoteNo ?? "-" }}</td>
                    <td class="px-3 py-2">{{ row.creditNoteDate ?? "-" }}</td>
                    <td class="px-3 py-2">
                      <div class="font-medium text-slate-800">{{ row.customerName ?? "-" }}</div>
                      <div class="text-xs text-slate-500">
                        {{ row.customerId ?? "" }}
                        <span v-if="row.customerType" class="ml-1 text-slate-400"
                    ...
                    <td class="px-3 py-2 text-right tabular-nums">
                      {{ currencyMyr(row.invoiceTotalAmount) }}
                    </td>
                    <td class="px-3 py-2 text-right tabular-nums">
                      {{ currencyMyr(row.invoiceBalanceAmount) }}
                    </td>
                    <td class="px-3 py-2 text-right tabular-nums font-medium">
                      {{ currencyMyr(row.creditNoteTotalAmount) }}
                    </td>
                    <td class="px-3 py-2">
                      <span
                        v-if="row.status"
                ...
                <tfoot v-if="rows.length > 0" class="bg-slate-50">
                  <tr class="border-t-2 border-slate-300 font-medium">
                    <td colspan="5" class="px-3 py-2 text-right text-xs uppercase text-slate-600">
                      Total
                    </td>
                    <td class="px-3 py-2 text-right tabular-nums">
                      {{ currencyMyr(footer.invoiceTotalAmount) }}
                    </td>
                    <td class="px-3 py-2 text-right tabular-nums">
                      {{ currencyMyr(footer.invoiceBalanceAmount) }}
                    </td>
                    <td class="px-3 py-2 text-right tabular-nums">
                      {{ currencyMyr(footer.creditNoteTotalAmount ?? 0) }}
                    </td>
                    <td colspan="2" />
                  </tr>
                </tfoot>
