...
import { listPettyCashRequests } from "@/api/cms";
import { useToast } from "@/composables/useToast";
import type { PettyCashRequestListRow } from "@/types";

const PAGE_NAME = "Request Petty Cash";
const PAGE_BREADCRUMB = "Petty Cash / Request Petty Cash";

const toast = useToast();

const rows = ref<PettyCashRequestListRow[]>([]);
...
const columns: FimsColumn<PettyCashRequestListRow>[] = [
  ...
  { key: "pmsRequestBy", label: "Request By ID", hideable: true, value: (r) => r.pmsRequestBy ?? "" },
  { key: "stfStaffName", label: "Request By Name", hideable: true, value: (r) => r.stfStaffName ?? "" },
  { key: "pmsRequestDate", label: "Request Date", sortable: true, sortKey: "pms_request_date", hideable: true, value: (r) => r.pmsRequestDate ?? "" },
  { key: "pmsTotalAmt", label: "Total Amount", hideable: true, align: "right", value: (r) => formatMoney(r.pmsTotalAmt) },
  { key: "pcbBatchId", label: "Recoup Batch", sortable: true, sortKey: "pcb_batch_id", hideable: true, value: (r) => r.pcbBatchId ?? "" },
  { key: "pcbStatus", label: "Recoup Status", sortable: true, sortKey: "pcb_status", hideable: true, value: (r) => r.pcbStatus ?? "" },
  { key: "recoupCreatedDate", label: "Recoup Date", hideable: true, hiddenByDefault: true, value: (r) => r.recoupCreatedDate },
  { key: "bimBillsNo", label: "Bill No", sortable: true, sortKey: "bim_bills_no", hideable: true, value: (r) => r.bimBillsNo ?? "" },
  { key: "bimStatus", label: "Bill Status", sortable: true, sortKey: "bim_status", hideable: true, value: (r) => r.bimStatus ?? "" },
  { key: "billCreatedDate", label: "Bill Date", hideable: true, hiddenByDefault: true, value: (r) => r.billCreatedDate },
  ...
  { key: "voucherCreatedDate", label: "Voucher Date", hideable: true, hiddenByDefault: true, value: (r) => r.voucherCreatedDate },
...
async function loadRows() {
  ...
  try {
    ...
    const res = await listPettyCashRequests(`?${params.toString()}`);
    rows.value = res.data;
...
const {
  ...
  pageName: PAGE_NAME,
  apiDataPath: "/petty-cash/requests",
  defaultExportColumns: ["Application No", "Request By ID", "Request By Name", "Request Date", "Total Amount", "Recoup Batch", "Recoup Status", "Bill No", "Bill Status", "Voucher No", "Voucher Status"],
      ...
      <article class="rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <div class="space-y-4 p-4">
          ...
          <FimsListTable
            ...
            :sort-dir="sortDir"
            :row-key="(r) => r.pmsId"
            :group-by="(r) => `Voucher Status ${r.vmaVchStatus ?? 'N/A'}`"
            min-width="1600px"
            @sort="onSort"
          >
            <template #action="{ row }">
              <a :href="(row as PettyCashRequestListRow).urlView" class="rounded p-1 text-slate-600 hover:bg-slate-100" title="View">
