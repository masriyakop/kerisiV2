...
/**
 * Account Receivable / Debit Note (PAGEID 1461, MENUID 1042)
 *
 * Source: FIMS BL `DT_AR_DEBIT_NOTE_LIST` — same shape as Credit Note list
 * (concatenated search, cascading delete). Scoped to `dnm_system_id='AR_DN'`.
 * Footer row ("Total Debit Note / Invoice Total / Invoice Balance") comes
 * from `meta.footer` on the controller response.
 */
...
import { deleteDebitNote, listDebitNotes } from "@/api/cms";
import { useToast } from "@/composables/useToast";
import { useConfirmDialog } from "@/composables/useConfirmDialog";
import type { DebitNoteRow, NoteListFooter } from "@/types";
...
type SortKey =
  ...
  | "cim_invoice_no"
  | "dnm_dn_total_amount";
const sortBy = ref<SortKey>("dnm_dnote_no");
...
async function loadRows() {
  ...
  try {
    const res = await listDebitNotes(`?${params.toString()}`);
    rows.value = res.data;
    ...
    footer.value = {
      ...
      debitNoteTotalAmount: Number(f.debitNoteTotalAmount ?? 0),
    };
  } catch (e) {
    toast.error("Load failed", e instanceof Error ? e.message : "Unable to load debit notes.");
  } finally {
    loading.value = false;
  }
}
...
async function handleDelete(row: DebitNoteRow) {
  const ok = await confirm({
    title: "Delete debit note?",
    message: `Delete debit note \u201C${row.debitNoteNo ?? row.id}\u201D and all its line items? This cannot be undone.`,
    confirmText: "Delete",
    destructive: true,
  });
  if (!ok) return;
  ...
  try {
    ...
    toast.success("Deleted", `Debit note ${row.debitNoteNo ?? row.id} removed.`);
    if (rows.value.length === 1 && page.value > 1) page.value -= 1;
    await loadRows();
  } catch (e) {
    toast.error("Delete failed", e instanceof Error ? e.message : "Unable to delete debit note.");
  }
}

function goCreate() {
  void router.push("/admin/kerisi/m/1783");
}

function goEdit(row: DebitNoteRow) {
  void router.push({ path: "/admin/kerisi/m/1783", query: { id: row.id, mode: "edit" } });
}

function goView(row: DebitNoteRow) {
  void router.push({ path: "/admin/kerisi/m/1783", query: { id: row.id, mode: "view" } });
}
...
const {
  ...
  pageName: "Debit Note",
  apiDataPath: "/account-receivable/debit-note",
  defaultExportColumns: exportColumns,
  getFilteredList: () =>
    rows.value.map((r) => ({
      "Debit Note No": r.debitNoteNo ?? "",
      "Debit Note Date": r.debitNoteDate ?? "",
      ...
      "Debit Note Amount": currencyMyr(r.debitNoteTotalAmount),
      Status: r.status ?? "",
    })),
  datatableRef,
  searchKeyword: q,
...
async function exportExcel() {
  try {
    ...
    a.href = url;
    a.download = `Debit_Note_${new Date().toISOString().slice(0, 10)}.xlsx`;
    a.click();
...
<template>
  <AdminLayout>
    <div class="mx-auto max-w-7xl space-y-4">
      ...
      />
      <p class="text-base font-semibold text-slate-500">Account Receivable / Debit Note</p>

      <article class="rounded-lg border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-100 px-4 py-3">
          <h1 class="text-base font-semibold text-slate-900">List of Debit Notes</h1>
              ...
              <table class="w-full min-w-[1100px] text-sm">
                <thead class="sticky top-0 bg-slate-50">
                  <tr class="border-b border-slate-200 text-left">
                    ...
                    <th
                      ...
                      @click="toggleSort('dnm_dnote_no')"
                    >
                      Debit Note No
                      <span v-if="sortBy === 'dnm_dnote_no'">{{ sortDir === "asc" ? "↑" : "↓" }}</span>
                    </th>
                    <th
                      class="cursor-pointer px-3 py-2 text-xs font-semibold uppercase"
                      @click="toggleSort('dnm_dnote_date')"
                    >
                      Date
                      <span v-if="sortBy === 'dnm_dnote_date'">{{ sortDir === "asc" ? "↑" : "↓" }}</span>
                    </th>
                    <th
                      class="cursor-pointer px-3 py-2 text-xs font-semibold uppercase"
                      @click="toggleSort('dnm_cust_name')"
                    >
                      Customer
                      <span v-if="sortBy === 'dnm_cust_name'">{{ sortDir === "asc" ? "↑" : "↓" }}</span>
                    </th>
                    <th
                      class="cursor-pointer px-3 py-2 text-xs font-semibold uppercase"
                    ...
                    <th
                      ...
                      @click="toggleSort('dnm_dn_total_amount')"
                    >
                      Debit Note Amount
                      <span v-if="sortBy === 'dnm_dn_total_amount'">{{ sortDir === "asc" ? "↑" : "↓" }}</span>
                    </th>
                ...
                <tbody>
                  ...
                  <tr v-for="row in rows" :key="row.id" class="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td class="px-3 py-2 font-medium text-slate-900">{{ row.debitNoteNo ?? "-" }}</td>
                    <td class="px-3 py-2">{{ row.debitNoteDate ?? "-" }}</td>
                    <td class="px-3 py-2">
                    ...
                    <td class="px-3 py-2 text-right tabular-nums font-medium">
                      {{ currencyMyr(row.debitNoteTotalAmount) }}
                    </td>
                    <td class="px-3 py-2">
                      <span
                        v-if="row.status"
                ...
                <tfoot v-if="rows.length > 0" class="bg-slate-50">
                  <tr class="border-t-2 border-slate-300 font-medium">
                    ...
                    <td class="px-3 py-2 text-right tabular-nums">
                      {{ currencyMyr(footer.debitNoteTotalAmount ?? 0) }}
                    </td>
                    <td colspan="2" />
                  </tr>
                </tfoot>
