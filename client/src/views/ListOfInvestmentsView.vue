 ...
 *
 * Source: FIMS BL `API_LIST_OF_NEW_INVESTMENT`
 * (action=listing_all_dt). Read-only datatable scoped to
 * ipf_status != 'DRAFT' with 12 columns and a 9-field smart filter
 * (Prefix, Batch No, Institution, Period From/To, Matured From/To,
 * Amount, Status).
 *
 * The Action column in the legacy UI has four buttons:
 *   1. Download Journal (investment.downloadjournal())
 *   2. Edit (menuID 3226)
 *   3. View Journal Details (menuID 3313)
 *   4. Cancel (legacy action=cancel_inv; investment_report_detail
 *      side-effects).
 *
 * None of those four flows are migrated yet, so each Action button
 * renders disabled with a tooltip until the follow-up migrations
 * land.
 */
...
import { getListOfInvestmentOptions, listInvestments } from "@/api/cms";
import { useToast } from "@/composables/useToast";
import type {
  ListOfInvestmentOptions,
  ListOfInvestmentRow,
  ListOfInvestmentSmartFilter,
} from "@/types";

const toast = useToast();
const datatableRef = ref<DatatableRefApi | null>(null);

const rows = ref<ListOfInvestmentRow[]>([]);
...
const smartFilter = ref<ListOfInvestmentSmartFilter>({
  prefix: "",
  batch: "",
  institution: "",
  periodFrom: "",
...
const options = ref<ListOfInvestmentOptions>({
  prefix: [],
  batchNo: [],
  institution: [],
  status: [],
...
async function loadOptions() {
  try {
    const res = await getListOfInvestmentOptions();
...
function isoToDmy(iso: string): string {
  // Convert <input type="date"> YYYY-MM-DD into dd/mm/yyyy (legacy server format).
  if (!iso) return "";
...
async function loadRows() {
  ...
  if (smartFilter.value.prefix)
    params.set("filter_prefix", smartFilter.value.prefix);
  ...
  if (smartFilter.value.maturedFrom)
    params.set("filter_matured_from", isoToDmy(smartFilter.value.maturedFrom));
  if (smartFilter.value.maturedTo)
    params.set("filter_matured_to", isoToDmy(smartFilter.value.maturedTo));
  ...
  try {
    const res = await listInvestments(`?${params.toString()}`);
    rows.value = res.data;
...
function institutionCell(row: ListOfInvestmentRow): string {
  const parts: string[] = [];
...
function investmentCell(row: ListOfInvestmentRow): string {
...
function journalCell(row: ListOfInvestmentRow): string {
...
function tenureCell(row: ListOfInvestmentRow): string {
  const parts: string[] = [];
  if (row.period !== null) {
...
function receiptCell(row: ListOfInvestmentRow): string {
...
const {
  ...
  pageName: "List of Investments",
  apiDataPath: "/investment/list",
  ...
  getFilteredList: () =>
    rows.value.map((r) => ({
      ...
      Status: r.status ?? "",
      "Withdrawal Type": r.withdrawalType ?? "",
    })),
  datatableRef,
  searchKeyword: q,
  smartFilter,
...
async function exportExcel() {
  try {
    ...
    const ws = wb.addWorksheet("List of Investments");
    ...
    a.href = url;
    a.download = `ListOfInvestments_${new Date().toISOString().slice(0, 10)}.xlsx`;
    a.click();
...
function withdrawalBadge(kind: string | null): string {
  switch (kind) {
    case "PREMATURED":
    ...
    <div class="mx-auto max-w-7xl space-y-4">
      ...
      <nav class="text-sm text-slate-500">
        <ol class="flex flex-wrap items-center gap-1">
          ...
          <li class="font-semibold text-slate-700">List of Investments</li>
        </ol>
      </nav>
      ...
      <article class="rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        >
          <h1 class="text-base font-semibold text-slate-900">List of Investments</h1>
          <button
            type="button"
              ...
              <table class="w-full min-w-[1500px] text-sm">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-3 py-2 whitespace-pre-line text-xs text-slate-600">
                      {{ receiptCell(row) }}
                    </td>
                    <td class="px-3 py-2">
                      <span
                    ...
                    <td class="px-3 py-2">
                      <span
                        v-if="row.withdrawalType"
                        class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium"
                        :class="withdrawalBadge(row.withdrawalType)"
                      >
                        {{ row.withdrawalType }}
                      </span>
                    ...
                    <td class="px-3 py-2">
                      <div class="flex items-center gap-1">
                        <button
                          type="button"
                          disabled
                          title="Download Journal (not yet migrated)"
                        ...
                        <button
                          type="button"
                          disabled
                          title="Edit (legacy menuID 3226 not yet migrated)"
                          class="cursor-not-allowed rounded p-1 text-slate-300"
                        >
                        ...
                        <button
                          type="button"
                          disabled
                          title="View Journal Details (legacy menuID 3313 not yet migrated)"
                          class="cursor-not-allowed rounded p-1 text-slate-300"
                        >
                        ...
                        <button
                          type="button"
                          disabled
                          title="Cancel Investment (not yet migrated — requires investment_report_detail handling)"
                          class="cursor-not-allowed rounded p-1 text-slate-300"
                        >
        ...
        >
          ...
          <div class="grid grid-cols-1 gap-4 p-4 sm:grid-cols-2">
            <div>
              ...
              <select
                v-model="smartFilter.prefix"
                class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
              >
                <option value="">Any</option>
                <option v-for="opt in options.prefix" :key="opt.id" :value="opt.id">
                  {{ opt.label }}
                </option>
              </select>
            </div>
            ...
            <div>
              <label class="mb-1 block text-sm font-medium text-slate-700">Matured From</label>
              <input
                v-model="smartFilter.maturedFrom"
                type="date"
                class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
              />
            </div>
            <div>
              <label class="mb-1 block text-sm font-medium text-slate-700">Matured To</label>
              <input
                v-model="smartFilter.maturedTo"
                type="date"
                class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
              />
            </div>
