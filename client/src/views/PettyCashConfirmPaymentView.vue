...
import { listPettyCashConfirmPaymentAwaiting, listPettyCashConfirmPaymentConfirmed } from "@/api/cms";
import { useToast } from "@/composables/useToast";
import type { PettyCashConfirmPaymentRow } from "@/types";

const PAGE_NAME = "Confirmation Payment";
const PAGE_BREADCRUMB = "Petty Cash / Confirmation Payment";

const toast = useToast();

type Tab = "awaiting" | "confirmed";
const tab = ref<Tab>("awaiting");

const rows = ref<PettyCashConfirmPaymentRow[]>([]);
...
const awaitingColumns = computed<FimsColumn<PettyCashConfirmPaymentRow>[]>(() => [
  ...
  { key: "pcbBatchAmt", label: "Batch Amount", hideable: true, align: "right", value: (r) => formatMoney(r.pcbBatchAmt) },
  { key: "pcbStatus", label: "Batch Status", sortable: true, sortKey: "pcb_status", hideable: true, value: (r) => r.pcbStatus ?? "" },
  { key: "pcbApproveAmt", label: "Approve Amount", hideable: true, align: "right", value: (r) => formatMoney(r.pcbApproveAmt) },
  { key: "vmaVoucherNo", label: "Voucher No", hideable: true, value: (r) => r.vmaVoucherNo ?? "" },
  { key: "vdePaymentNo", label: "Payment No", hideable: true, value: (r) => r.vdePaymentNo ?? "" },
  { key: "preTotalAmt", label: "Payment Amount", hideable: true, align: "right", value: (r) => formatMoney(r.preTotalAmt) },
  { key: "action", label: "Action" },
]);

const confirmedColumns = computed<FimsColumn<PettyCashConfirmPaymentRow>[]>(() => [
  { key: "no", label: "No", value: (r) => r.index },
  { key: "pcbBatchId", label: "Batch No", sortable: true, sortKey: "pcb_batch_id", hideable: true, value: (r) => r.pcbBatchId ?? "" },
  { key: "pcbBatchAmt", label: "Batch Amount", hideable: true, align: "right", value: (r) => formatMoney(r.pcbBatchAmt) },
  { key: "pcbStatus", label: "Batch Status", sortable: true, sortKey: "pcb_status", hideable: true, value: (r) => r.pcbStatus ?? "" },
  ...
  { key: "vdePaymentNo", label: "Payment No", hideable: true, value: (r) => r.vdePaymentNo ?? "" },
  { key: "preTotalAmt", label: "Payment Amount", hideable: true, align: "right", value: (r) => formatMoney(r.preTotalAmt) },
  { key: "stfStaffName", label: "Received By", hideable: true, value: (r) => r.stfStaffName ?? "" },
  { key: "pcbReceivedate", label: "Received Date", hideable: true, value: (r) => r.pcbReceivedate ?? "" },
  { key: "action", label: "Action" },
]);

const activeColumns = computed(() => (tab.value === "awaiting" ? awaitingColumns.value : confirmedColumns.value));
...
async function loadRows() {
  ...
  try {
    ...
    const res = tab.value === "awaiting"
      ? await listPettyCashConfirmPaymentAwaiting(`?${params.toString()}`)
      : await listPettyCashConfirmPaymentConfirmed(`?${params.toString()}`);
    rows.value = res.data;
...
const {
  ...
  pageName: PAGE_NAME,
  apiDataPath: "/petty-cash/confirm-payment",
  defaultExportColumns: ["Batch No", "Batch Amount", "Batch Status", "Approve Amount", "Voucher No", "Payment No", "Payment Amount"],
...
async function exportExcel() {
  try {
    ...
    a.href = url;
    a.download = `${PAGE_NAME.replace(/\s+/g, "_")}_${tab.value}_${new Date().toISOString().slice(0, 10)}.xlsx`;
    a.click();
  ...
  <AdminLayout>
    <div class="space-y-4">
      ...
      <div class="inline-flex rounded-lg border border-slate-200 bg-white p-1 shadow-sm">
        ...
        <button type="button" class="rounded-md px-3 py-1.5 text-xs font-medium" :class="tab === 'confirmed' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'" @click="switchTab('confirmed')">Confirmed</button>
      </div>

      <article class="rounded-lg border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between gap-2 border-b border-slate-100 px-4 py-3">
          <h1 class="text-base font-semibold text-slate-900">{{ PAGE_NAME }} — {{ tab === 'awaiting' ? 'Awaiting' : 'Confirmed' }}</h1>
        ...
        <div class="space-y-4 p-4">
          ...
          <FimsListTable
            ...
            :sort-dir="sortDir"
            :row-key="(r) => r.pcbId"
            :group-by="(r) => `Status ${r.pcbStatus ?? ''}`"
            min-width="1100px"
            @sort="onSort"
          >
            <template #action="{ row }">
              <a :href="(row as PettyCashConfirmPaymentRow).urlView" class="rounded p-1 text-slate-600 hover:bg-slate-100" title="View">
