...
import { ChevronUp, ChevronDown, Columns3, FileText } from "lucide-vue-next";
import type { DatatableExportConfig, DatatableTemplateState } from "@/composables/useDatatableFeatures";
import type { VcTncRow } from "@/types";

type ColumnKey = "no" | "ounCode" | "ounDesc" | "stStaffIdHead" | "stStaffIdSuperior" | "action";
...
const columns = ref<ColumnDef[]>([
  { key: "no", label: "No" },
  { key: "ounCode", label: "Code", sortable: true, hideable: true },
  { key: "ounDesc", label: "Description", sortable: true, hideable: true },
  { key: "stStaffIdHead", label: "Head of Department", sortable: true, hideable: true },
  { key: "stStaffIdSuperior", label: "VC/TNC", sortable: true, hideable: true },
  { key: "action", label: "Action" },
]);

const hiddenKeys = ref<Set<ColumnKey>>(new Set());
const sortColumn = ref<ColumnKey>("ounCode");
...
const sortedRows = computed(() => {
  ...
  const valueOf = (r: VcTncRow): string | number => {
    if (col === "ounCode") return r.ounCode ?? "";
    if (col === "ounDesc") return r.ounDesc ?? "";
    if (col === "stStaffIdHead") return r.stStaffIdHeadLabel ?? r.stStaffIdHead ?? "";
    if (col === "stStaffIdSuperior") return r.stStaffIdSuperiorLabel ?? r.stStaffIdSuperior ?? "";
    return r.index;
  };
...
function displayValue(row: VcTncRow, key: ColumnKey): string | number {
  if (key === "no") return row.index;
  if (key === "ounCode") return row.ounCode ?? "";
  if (key === "ounDesc") return row.ounDesc ?? "";
  if (key === "stStaffIdHead") return row.stStaffIdHeadLabel ?? row.stStaffIdHead ?? "";
  if (key === "stStaffIdSuperior") return row.stStaffIdSuperiorLabel ?? row.stStaffIdSuperior ?? "";
  return "";
}
...
function applyTemplateState(t: Partial<DatatableTemplateState> & { columnOrder?: string[]; hiddenColumns?: string[] }) {
  ...
  if (t.sortColumn) sortColumn.value = t.sortColumn as ColumnKey;
...
function getExportConfig(): DatatableExportConfig | null {
  ...
  const keyToLabel: Record<Exclude<ColumnKey, "no" | "action">, string> = {
    ounCode: "Code",
    ounDesc: "Description",
    stStaffIdHead: "Head of Department",
    stStaffIdSuperior: "VC/TNC",
  };
  const keyToValue: Record<Exclude<ColumnKey, "no" | "action">, (r: VcTncRow) => string> = {
    ounCode: (r) => r.ounCode ?? "",
    ounDesc: (r) => r.ounDesc ?? "",
    stStaffIdHead: (r) => r.stStaffIdHeadLabel ?? r.stStaffIdHead ?? "",
    stStaffIdSuperior: (r) => r.stStaffIdSuperiorLabel ?? r.stStaffIdSuperior ?? "",
  };

  const columnKeys = exportColKeys as Exclude<ColumnKey, "no" | "action">[];
  const columnsOut = columnKeys.map((k) => keyToLabel[k]);
  const data = sortedRows.value.map((r) => {
    const row: Record<string, unknown> = {};
    columnKeys.forEach((k) => {
      row[keyToLabel[k]] = keyToValue[k](r);
    });
    return row;
  });
    ...
    <div class="overflow-x-auto rounded-lg border border-slate-200">
      <div :class="hasScrollableBody ? 'max-h-[420px] overflow-y-auto' : ''">
        <table class="w-full min-w-[960px] text-sm">
          ...
          <tbody>
            <template v-for="(row, idx) in pagedRows" :key="row.id">
              <tr v-if="showGroupBar(idx)" class="bg-slate-100/80">
                <td :colspan="visibleColumns.length" class="px-3 py-1 text-xs font-semibold text-slate-600">
                  VC/TNC {{ row.stStaffIdSuperior || "— (None)" }}
                </td>
              </tr>
              <tr class="border-b border-slate-100 hover:bg-slate-50">
                <td v-for="col in visibleColumns" :key="col.key" class="px-3 py-2">
                  ...
                  <template v-else>{{ displayValue(row, col.key) }}</template>
                </td>
              </tr>
            </template>
