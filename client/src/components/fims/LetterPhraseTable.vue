...
import { ChevronUp, ChevronDown, Columns3, Pencil } from "lucide-vue-next";
import type { DatatableExportConfig, DatatableTemplateState } from "@/composables/useDatatableFeatures";
import type { LetterPhraseRow } from "@/types";

type ColumnKey = "no" | "lpmValue" | "lpmValueDescBm" | "lpmValueDesc" | "lpmCode" | "action";
...
// dt_bi labels carried over from legacy DatatableColumnDetails; dt_key maps to the
// Eloquent column returned from the API (camelCased by CamelCaseMiddleware).
const columns = ref<ColumnDef[]>([
  { key: "no", label: "No" },
  { key: "lpmValue", label: "Value", sortable: true, hideable: true },
  { key: "lpmValueDescBm", label: "Value Desc BM", sortable: true, hideable: true },
  { key: "lpmValueDesc", label: "Value Desc", sortable: true, hideable: true },
  { key: "lpmCode", label: "Code", sortable: true, hideable: true },
  { key: "action", label: "Action" },
]);

const hiddenKeys = ref<Set<ColumnKey>>(new Set());
const sortColumn = ref<ColumnKey>("lpmValue");
...
const sortedRows = computed(() => {
  ...
  const valueOf = (r: LetterPhraseRow): string | number => {
    if (col === "lpmValue") return r.lpmValue ?? "";
    if (col === "lpmValueDescBm") return r.lpmValueDescBm ?? "";
    if (col === "lpmValueDesc") return r.lpmValueDesc ?? "";
    if (col === "lpmCode") return r.lpmCode ?? "";
    return r.index;
  };
...
function valueForKey(row: LetterPhraseRow, key: ColumnKey): string | number {
  if (key === "no") return row.index;
  if (key === "lpmValue") return row.lpmValue ?? "";
  if (key === "lpmValueDescBm") return row.lpmValueDescBm ?? "";
  if (key === "lpmValueDesc") return row.lpmValueDesc ?? "";
  if (key === "lpmCode") return row.lpmCode ?? "";
  return "";
}
...
function getExportConfig(): DatatableExportConfig | null {
  ...
  const keyToLabel: Record<Exclude<ColumnKey, "no" | "action">, string> = {
    lpmValue: "Value",
    lpmValueDescBm: "Value Desc BM",
    lpmValueDesc: "Value Desc",
    lpmCode: "Code",
  };
  const keyToValue: Record<Exclude<ColumnKey, "no" | "action">, (r: LetterPhraseRow) => string> = {
    lpmValue: (r) => r.lpmValue ?? "",
    lpmValueDescBm: (r) => r.lpmValueDescBm ?? "",
    lpmValueDesc: (r) => r.lpmValueDesc ?? "",
    lpmCode: (r) => r.lpmCode ?? "",
  };
    ...
    <div class="overflow-x-auto rounded-lg border border-slate-200">
      <div :class="hasScrollableBody ? 'max-h-[420px] overflow-y-auto' : ''">
        <table class="w-full min-w-[720px] text-sm">
          ...
          <tbody>
            <template v-for="(row, idx) in pagedRows" :key="row.lpmValue">
